...
void main() {
  ...
  group('REAL Upload → Publish E2E Test (NO MOCKS)', () {
    ...
    setUp(() async {
      ...
      testVideoFile = File('${tempDir.path}/test_real_e2e_video_${DateTime.now().millisecondsSinceEpoch}.mp4');
      ...
      if (!authResult.success) {
        throw Exception('Failed to authenticate test user: ${authResult.errorMessage}');
      }

      print('✅ Test user authenticated: ${authService.currentPublicKeyHex?.substring(0, 8)}...');
      ...
      print('   Note: Relay connections are asynchronous - publishing will connect as needed');
    });
    ...
    test('REAL E2E: Upload to real Blossom server → Publish to real Nostr relays',
        () async {
      print('\n🎬 === STARTING REAL E2E TEST (NO MOCKS) ===\n');
      print('⚠️  This test uploads to real Blossom CDN and real Nostr relays!');
      print('⚠️  Test may fail if Blossom server is down or not configured\n');
      ...
      try {
        upload = await uploadManager.startUpload(
          ...
          description: 'This is a REAL test video uploaded to actual Blossom CDN',
          hashtags: ['real-e2e-test', 'integration-test'],
          videoDuration: const Duration(seconds: 5),
        );
      } catch (e) {
        print('❌ Upload failed (this is expected if Blossom server is not configured):');
        print('   Error: $e');
      ...
      expect(upload.cdnUrl, startsWith('https://'),
      ...
      bool publishSuccess;
      try {
        publishSuccess = await videoEventPublisher.publishDirectUpload(upload);
      } catch (e, stackTrace) {
      ...
      expect(publishedUpload!.nostrEventId, isNotNull,
      ...
      expect(publishedUpload.status, equals(UploadStatus.published),
          reason: 'Upload should be marked as published');
      expect(publishedUpload.completedAt, isNotNull,
      ...
      if (upload.thumbnailPath != null) {
        print('✅ Thumbnail uploaded to REAL Blossom CDN: ${upload.thumbnailPath}');
      }
      print('✅ Nostr event published to REAL relays: ${publishedUpload.nostrEventId}');
    ...
    }, timeout: const Timeout(Duration(minutes: 5))); // Longer timeout for real network operations

    test('REAL E2E: Verify uploaded video is retrievable from CDN', () async {
      ...
      try {
        ...
        // Verify it's a video
        expect(contentType, contains('video'),
